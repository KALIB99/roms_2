  ...
  <div id="user-edit-tab-info">
    <div class="vx-row">
      <div class="vx-col w-full md:w-1/2">
        ...
        <!-- Col Content -->
        <div>
            <vs-input class="w-full mt-4" label="FirstName"  v-model="data_local.firstname" v-validate="'required|alpha_spaces'" name="firstname" />
            <span class="text-danger text-sm"  v-show="errors.has('firstname')">{{ errors.first('firstname') }}</span>

            <vs-input class="w-full mt-4" label="LastName" v-model="data_local.lastname"  v-validate="'required|alpha_spaces'" name="lastname" />
        ...
        <div class="mt-4">
          <label class="vs-input--label">Faculty</label>
          <v-select v-model="status_local" :clearable="false" :options="statusOptions" v-validate="'required'" name="status" :dir="$vs.rtl ? 'rtl' : 'ltr'" />
          <span class="text-danger text-sm"  v-show="errors.has('status')">{{ errors.first('status') }}</span>
        </div>
          <!-- Gender -->
...
export default {
  ...
  data () {
    return {
      ...
      data_local: JSON.parse(JSON.stringify(this.data.user)),
