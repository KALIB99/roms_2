...
mock.onGet('/api/research/products').reply(() => {
  return [200, JSON.parse(JSON.stringify(data.products)).reverse()]
})

// POST : Add new Item
mock.onPost('/api/research/products/').reply((request) => {
...
// Update Product
mock.onPost(/\/api\/research\/products\/\d+/).reply((request) => {
...
// DELETE: Remove Item
mock.onDelete(/\/api\/research\/products\/\d+/).reply((request) => {
